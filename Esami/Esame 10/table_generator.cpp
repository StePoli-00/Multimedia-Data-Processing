...
int main() 
{
	...
	// TODO
	//std::vector<huffman::elem>vec=huff.table();

	std::vector<huffman<uint8_t>::elem> vec=huff.table();
	...
	//for (auto& x : vec)
	//{
	//	os.write(reinterpret_cast<char*> (&x._sym), sizeof(uint8_t));
	//	os.write(reinterpret_cast<char*> (&x._len), sizeof(uint8_t));
	//	os.write(reinterpret_cast<char*> (&x._code), sizeof(uint32_t));
	...
	hufstr h;
	std::vector<uint8_t>compress=h.compress("aeio");
