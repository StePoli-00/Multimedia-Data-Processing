		...
		else //Data message
		{
			...
			if (d.local_message_type != 16)
			{
				for (auto& f : d.fields)
				{
					...
					if (f.size == 1)
					{
						is.read(reinterpret_cast<char*>(&tmp.val8_), f.size);
					}
					else if (f.size == 2)
					{
						is.read(reinterpret_cast<char*>(&tmp.val16_), f.size);
					}
					else  //qualcosa in questo else if fa sballare tutto
					{
						is.read(reinterpret_cast<char*>(&tmp.val32_),f.size);
					}
				
					//else{
					// //il CRC viene corretto ma dopo c'è la corruzione
					// is.read(reinterpret_cast<char*>(&tmp.val32_), f.size);
					// }
