...
bool read_header(std::istream& is, mat<vec3b> &img, int &total_bytes)
{
...
bool load_pcx(const std::string& filename, mat<vec3b>& img)
{
	...
	if (!is) { return false; }
	int total_bytes;
	bool res=read_header(is, img, total_bytes);
	...
	for (int r = 0; r < img.rows(); r++)
	{
		...
		//std::vector<uint8_t> scanline_r(dim), scanline_g(dim), scanline_b(dim);
		DecodeRunLenght(is, scanline, total_bytes);
		//DecodeRunLenght(is, scanline_r, dim);
		//DecodeRunLenght(is, scanline_g, dim);
		//DecodeRunLenght(is, scanline_b, dim);
		for (int c = 0; c < img.cols(); c++)
		{
			
			
				/*img(r, c)[0] = scanline_r[c];
				img(r, c)[1] = scanline_g[c];
				img(r, c)[2] = scanline_b[c];*/
				img(r, c)[0] = scanline[c];
				img(r, c)[1] = scanline[c+dim];
				img(r, c)[2] = scanline[c+dim*2]; 
